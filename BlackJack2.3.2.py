"""
Created on Mon April 25 6:26:47 2023
...
def player_hand_21_2(shuffle, cards_delt, player_info, dealer_info, i, error, choices):
    player_cards = player_info['player_hand ' + str(i + 1)]['player_cards']
    player_sum = sum_cards(player_cards)
    ...
    while player_sum < 21:
        ...
        if decision == "1":
            print('')
            return player_info, cards_delt, decision, error, choices
...
def player_hand_21_1(player_info, dealer_info, shuffle, cards_delt, i, bankroll, insurance_d, error, choices):
    player_cards = player_info['player_hand ' + str(i + 1)]['player_cards']
    player_sum = sum_cards(player_cards)
    bet = player_info['player_hand ' + str(i + 1)]['bet']
    print(f'player cards     :{player_cards}')
    ...
    # stand
    if decision == "1":
        return player_info, cards_delt, bankroll, decision, i, error, choices

    # hit
    if decision == "2":
        player_cards, player_sum, cards_delt = player_hit(shuffle, cards_delt, player_cards)
        player_info['player_hand ' + str(i + 1)] = {'player_sum': player_sum, 'player_cards': player_cards, 'bet': bet}
        player_info, cards_delt, decision, error, choices = player_hand_21_2(shuffle, cards_delt, player_info, dealer_info, i, error, choices)
        return player_info, cards_delt, bankroll, decision, i, error, choices
...
def play_game(player_info, dealer_info, shuffle, cards_delt, bankroll, error, choices):
    i = 0
    hands = len(player_info)
    insurance_d = 0
    while i < hands:
        player_info, cards_delt, bankroll, decision, i, error, choices = player_hand_21_1(player_info, dealer_info,
                                                                         shuffle, cards_delt, i, bankroll, insurance_d,
                                                                         error, choices)
        print('')
        player_cards = player_info["player_hand " + str(i + 1)]["player_cards"]
        print(f"player cards     :{player_cards}")
        i += 1
    return player_info, cards_delt, bankroll, error, choices
...
def game_21(amount_of_decks, bankroll):
    ...
    while cards_delt < (len(shuffle) - randint(30, 52)):
        ...
        if choices == 0:
            ...
            error_percent = 100 * (choices- error)/choices
        ...
        print(f"playing percent  :{error_percent}")
        print(f"player bankroll  :{bankroll}")
        print("")
        print("New Hand")
        ...
        player_info, cards_delt, bankroll, error, choices = play_game(player_info, dealer_info, shuffle, cards_delt, bankroll, error, choices)
