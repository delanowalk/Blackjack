...
"""
Created on Thur May 18 8:49:38 2023
...
def player_hand_21_2(shuffle, cards_delt, player_info, i):
...
def play_game(player_info, dealer_info, shuffle, cards_delt, bankroll):
    ...
    while i < hands:
        player_info, cards_delt, bankroll, decision, i = player_hand_21_1(player_info, dealer_info, shuffle, cards_delt, i, bankroll)
        print('')
        i += 1
    return player_info, cards_delt
...
def player_hand_21_1(player_info, dealer_info, shuffle, cards_delt, i, bankroll):
    player_cards = player_info['player_hand ' + str(i + 1)]['player_cards']
    player_sum = sum_cards(player_cards)
    bet = player_info['player_hand ' + str(i+1)]['bet']
    print(f'player cards     :{player_cards}')
    ...
    if decision == "2":
        ...
        player_info, cards_delt, decision = player_hand_21_2(shuffle, cards_delt, player_info, i)
        return player_info, cards_delt, bankroll, decision, i
    ...
    # double
        ...
        player_cards, player_sum, cards_delt = player_hit(shuffle, cards_delt, player_cards)
        bet = player_info['player_hand ' + str(i + 1)]['bet']
        player_info['player_hand ' + str(i + 1)] = {'player_sum': player_sum, 'player_cards': player_cards, 'bet': int(bet)*2}
        print(f'player cards     :{player_cards}')
        print(f'player sum       :{player_sum}')
        return player_info, cards_delt, bankroll, decision, i
    ...
    # split
        ...
        if fucken_idiot == 100:
            print('Fuck you, you can not split when the cards are different you are the problem')
            player_info, cards_delt, bankroll, decision, i = player_hand_21_1(player_info, dealer_info, shuffle, cards_delt, i, bankroll)
            return player_info, cards_delt, bankroll, decision, i
        elif fucken_idiot == 0:
            player_info, cards_delt, bankroll, decision, i = player_hand_21_1(player_info, dealer_info, shuffle, cards_delt, i, bankroll)
            player_info, cards_delt, bankroll, decision, j = player_hand_21_1(player_info, dealer_info, shuffle, cards_delt, len(player_info)-1, bankroll)
            return player_info, cards_delt, bankroll, decision, i
